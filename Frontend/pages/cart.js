...
const Cart = () => {
  ...
  const fetchCartData = async () => {
    try {
      const response = await axios.get('http://my_store.test/api/orders', { headers });
  ...
  const handleRemoveItem = async (productId, orderId) => {
    try {
      const response = await axios.delete(`http://my_store.test/api/deletefromorder/${orderId}/product/${productId}`, { headers }); 
      if (response.status === 200) {
        console.log('ok')
        fetchCartData()
      }
